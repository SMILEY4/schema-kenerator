...
fun objectType(id: TypeId, block: ObjectTypeDataDsl.() -> Unit): ObjectTypeData {
    return ObjectTypeDataDsl(id).apply(block).build()
}

class ObjectTypeDataDsl(val id: TypeId) {

    private val data = ObjectTypeData(
        id = id,
        simpleName = "",
        qualifiedName = "",
    )
