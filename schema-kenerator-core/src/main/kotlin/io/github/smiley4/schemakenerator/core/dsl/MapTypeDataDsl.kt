...
fun mapType(id: TypeId, block: MapTypeDataDsl.() -> Unit): MapTypeData {
    return MapTypeDataDsl(id).apply(block).build()
}

class MapTypeDataDsl(val id: TypeId) {

    private val data = MapTypeData(
        id = id,
        simpleName = "",
        qualifiedName = "",
    ...
    private var keyTypeParameterName = ""
    private var valueTypeParameterName = ""
    ...
    /**
     * Sets the key type-parameter of this map. The type parameter with the name must exist as a type-parameter of this map.
     */
    fun key(name: String) {
        keyTypeParameterName = name
    }

    /**
     * Sets the value type-parameter of this map. The type parameter with the name must exist as a type-parameter of this map.
     */
    fun value(name: String) {
        valueTypeParameterName = name
    }

    fun build() = data.also { data ->
        data.typeParameters[keyTypeParameterName]?.also { typeParam ->
            data.keyType = PropertyData(
                name = "key",
                type = typeParam.type,
        ...
        data.typeParameters[valueTypeParameterName]?.also { typeParam ->
            data.valueType = PropertyData(
                name = "value",
                type = typeParam.type,
                nullable = typeParam.nullable,
                visibility = Visibility.PUBLIC,
